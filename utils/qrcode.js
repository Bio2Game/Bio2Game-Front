...
class QRCode {
  constructor(canvas, content) {
    const type = this._getTypeNumber(content)
    this.qrcode = new QRCodeModel(type, 2)
  ...
  draw() {
    ...
    const logoX = this.canvas.width / 2 - this.props.logoSize / 2
    const logoY = this.canvas.width / 2 - this.props.logoSize / 2
    ...
    for (let y = 0; y < length; y++) {
      for (let x = 0; x < length; x++) {
        ...
        const overlapsWithLogo =
          ...
          py + ysize >= logoY &&
          py - ysize <= logoY + logoSize

        if (!overlapsWithLogo && modules[x][y]) {
          this.drawPiece(ctx, x, y, modules, this.props)
        }
      }
    }
    ...
    ctx.drawImage(
      img,
      this.canvas.width / 2 - this.props.logoSize / 2,
      this.canvas.width / 2 - this.props.logoSize / 2,
      this.props.logoSize,
      this.props.logoSize,
    )
  ...
  drawPiece(ctx, x, y, modules, { size, padding }) {
    const gsize = Math.round(size / (modules.length + 2 * padding))
    if (
      (x < 7 && y < 7) ||
      (x < 7 && modules.length - y < 8) ||
      (modules.length - x < 8 && y < 7) ||
      (y != 0 && modules[x][y - 1]) ||
      (x != 0 && modules[x - 1][y]) ||
      (x != modules.length - 1 && modules[x + 1][y]) ||
      (y != modules.length - 1 && modules[x][y + 1])
    ) {
      ctx.fillRect((x + padding) * gsize, (y + padding) * gsize, gsize, gsize)
    } else {
      ctx.beginPath()
      ctx.arc(
        (x + padding + 0.5) * gsize,
